package BaekJoon;

import java.io.BufferedReader;
import java.io.InputStreamReader;
import java.util.StringTokenizer;

/*
	Question : 0에서부터 9까지의 숫자로 이루어진 N개의 숫자가 나열된 수열이 있다. 
	그 수열 안에서 연속해서 커지거나(같은 것 포함), 혹은 연속해서 작아지는(같은 것 포함) 수열 중 가장 길이가 긴 것을 찾아내어 그 길이를 출력하는 프로그램을 작성하라. 

	예를 들어 수열 1, 2, 2, 4, 4, 5, 7, 7, 2 의 경우에는 1 ≤ 2 ≤ 2 ≤ 4 ≤ 4 ≤ 5 ≤ 7 ≤ 7 이 가장 긴 구간이 되므로 그 길이 8을 출력한다. 
	수열 4, 1, 3, 3, 2, 2, 9, 2, 3 의 경우에는 3 ≥ 3 ≥ 2 ≥ 2 가 가장 긴 구간이 되므로 그 길이 4를 출력한다. 
	또 1, 5, 3, 6, 4, 7, 1, 3, 2, 9, 5 의 경우에는 연속해서 커지거나 작아지는 수열의 길이가 3 이상인 경우가 없으므로 2를 출력하여야 한다.
	
	input			 
	9
	1 2 2 4 4 5 7 7 2
		
	output
	8
	
	Solution : 1. 이전과 자기자신을 비교하면서 값이 커지는지 작아지는지 판정한다.
	 		   2. 판정하면서 Max값을 계속 업데이트 시킨다. 증가할때의 Max와 감소할때의 Max를 비교하도록 한다.

*/

public class BaekJoon_2491 {

	public static void main(String[] args) throws Exception {
		// TODO Auto-generated method stub
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		int N = Integer.parseInt(br.readLine());
		
		int max = 1;
		int increaseCount = 1;
		int decreaseCount = 1;
		
		StringTokenizer st = new StringTokenizer(br.readLine());
		int previous = Integer.parseInt(st.nextToken());
		
		for(int i=1; i<N; i++) {
			int num = Integer.parseInt(st.nextToken());
			
			increaseCount = (previous <= num) ? increaseCount+1 : 1;
			decreaseCount = (previous >= num) ? decreaseCount+1 : 1;
			
			max = (increaseCount > decreaseCount) ? Math.max(max, increaseCount) : Math.max(max, decreaseCount);
			previous = num;
		}
		
		System.out.println(max);
		
	}

}
