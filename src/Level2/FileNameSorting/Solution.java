 ...
 * 
 * 
 * 	Solution : 1. 배열 자체를 Sort하는 문제
 * 			   2. 숫자를 기준으로 split하여 0번째 배열에 오는 문자 -> head (toLowerCase를 통해 파일명의 대소문자 구분 무효화)
 * 			   3. h1, h2가 같을 경우 -> h1.compareTo(h2) == 0 는 길이만큼 substring 해준다 
 * 			   4. 숫자인 케이스를 뽑아서 다시한번 오름차순으로 정렬 -> findNum에서 리턴된 String 값
 * 			   5. 같지 않으면 헤더만 비교해서 정렬
 * 			   6. return 해줌
 * */
