 ...
 * 
 *  progress : -1, +100이 적힌 버튼을 4번, +10이 적힌 버튼을 5번, -1000이 적힌 버튼을 3번 누르면 0층에 도착 할 수 있습니다. 그러므로 16을 return 합니다.
 *	result : 16 -> 6 / 2554 -> 16 
 * 
 * 	Solution : 1. 일의 자리가 5보다 크면 (10 - 일의자리) 만큼 증가시켜주는게 횟수가 적다.
 * 			   2. 마찬가지로 일의자리가 5보다 작으면 해당 자리만큼 증가시켜주는게 횟수가 적다.
 * 			   3. 일의 자리가 5이면 무조건 5회의 증감은 일어난다.
 * 			   4. 위의 경우를 대입하여 10의 자리숫자가 5이상이면 다음 카운트 측정에 +1을 더해주기 위해 +5를 더한다.
 * 			   5. 로직이 끝나면 10씩 나누어 자릿수만큼 횟수 계산을 진행한다.
 * 			    
 * */
...
public class Solution {
    ...
    public int solution(int storey) {
        ...
        while(storey > 0) {
        	int oneDig = storey % 10;
        	int secDig = (storey / 10) % 10;
