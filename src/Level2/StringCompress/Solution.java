 ...
 * 
 * 	Solution : 1. 갯수만큼 쪼개서 반복을 하면서 같으면 앞에 숫자로 바꿔주는 형태
 * 			   2. 최종적으론 숫자+문자 조합의 총 갯수에서 최소 값을 구하는 것
 * 
 * */
...
public class Solution {
	...
	public int solution(String s) {
		...
		for(int i=1; i<=s.length()/2; i++) {
			String str = s.substring(0, i); // 0 ~ i번째까지 자름
			String comStr = "";
			String totalStr = "";
			int cnt = 1;
			...
			for(int j=i; j<s.length(); j+=i) {
				if((j+i) < s.length()) {
					comStr = s.substring(j, j+i);
 					...
 					comStr = s.substring(j, s.length()); // 남은 단건
 				}
